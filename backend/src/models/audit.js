...
AuditSchema.statics.getAudits = (isAdmin, userId, filters) => {
	return new Promise((resolve, reject) => {
		...
		query.select(
			"id name language creator collaborators company createdAt state"
		);
		query
			.exec()
			.then((rows) => {
...
AuditSchema.statics.updateGeneral = (isAdmin, auditId, userId, update) => {
	return new Promise(async (resolve, reject) => {
		if (update.company && update.company.name) {
			...
			try {
				update.company = await Company.create({
				...
				.then((row) => {
					if (!row)
						throw {
							fn: "NotFound",
							message:
								"Audit not found or Insufficient Privileges",
						};
					else {
						var sortOption = row.sortFindings.find(
							(e) =>
								e.category ===
								(finding.category || "No Category")
						);
						if ((sortOption && sortOption.sortAuto) || !sortOption)
							...
							return Audit.updateSortFindings(
								isAdmin,
								auditId,
								userId,
								null
			...
			.then((row) => {
				...
				if (finding === null)
					...
					var sortOption = row.sortFindings.find(
						(e) =>
							e.category ===
							(newFinding.category || "No Category")
					);
		...
		query
			...
			.then((row) => {
				...
				var findingList = _.chain(audit.findings)
					...
					.sort(
						(a, b) =>
							categoriesOrder.indexOf(a[0]) -
							categoriesOrder.indexOf(b[0])
					)
					.fromPairs()
					.map((value, key) => {
				...
				findingList.forEach((group) => {
					...
					var tmpFindings = group.findings.sort((a, b) => {
						...
						if (
							cvssA.success &&
							group.sortOption.sortValue === "cvssScore"
						)
...
AuditSchema.statics.updateVulnChart = (isAdmin, auditId, userId, newChart) => {
	return new Promise((resolve, reject) => {
		var sortAuto = true;
