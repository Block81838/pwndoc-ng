...
async function generateDoc(audit) {
	var templatePath = `${__basedir}/../report-templates/${
		audit.template.name
...
expressions.filters.select = function (input, attr) {
	return input.map(function (item) {
		return _getPropertyValue(item, attr);
	});
};
...
expressions.filters.sortArrayByField = function (input, field, order) {
	...
	const sorted = input.sort((a, b) => {
		...
		return (
			_.get(a, field)
				.toString()
				.localeCompare(_.get(b, field).toString(), undefined, {
					numeric: true,
				}) * order
		);
	});
...
async function prepAuditData(data, settings) {
	...
	var lowColorRemediationComplexity =
		settings.report.public.remediationColorsComplexity.lowColor.replace(
			"#",
			""
		);
	var mediumColorRemediationComplexity =
		settings.report.public.remediationColorsComplexity.mediumColor.replace(
			"#",
			""
		);
	var highColorRemediationComplexity =
		settings.report.public.remediationColorsComplexity.highColor.replace(
			"#",
			""
		);

	var cellLowColorRemediationComplexity =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		lowColorRemediationComplexity +
		'"/></w:tcPr>';
	var cellMediumColorRemediationComplexity =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		mediumColorRemediationComplexity +
		'"/></w:tcPr>';
	var cellHighColorRemediationComplexity =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		highColorRemediationComplexity +
		'"/></w:tcPr>';

	/** Remediation priority Colors for table cells */

	var lowColorRemediationPriority =
		settings.report.public.remediationColorsPriority.lowColor.replace(
			"#",
			""
		);
	var mediumColorRemediationPriority =
		settings.report.public.remediationColorsPriority.mediumColor.replace(
			"#",
			""
		);
	var highColorRemediationPriority =
		settings.report.public.remediationColorsPriority.highColor.replace(
			"#",
			""
		);
	var urgentColorRemediationPriority =
		settings.report.public.remediationColorsPriority.urgentColor.replace(
			"#",
			""
		);

	var cellLowColorRemediationPriority =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		lowColorRemediationPriority +
		'"/></w:tcPr>';
	var cellMediumColorRemediationPriority =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		mediumColorRemediationPriority +
		'"/></w:tcPr>';
	var cellHighColorRemediationPriority =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		highColorRemediationPriority +
		'"/></w:tcPr>';
	var cellUrgentColorRemediationPriority =
		'<w:tcPr><w:shd w:val="clear" w:color="auto" w:fill="' +
		urgentColorRemediationPriority +
		'"/></w:tcPr>';
	...
	// result.vuln_chart = await splitHTMLParagraphs(data.vuln_chart) || undefined
	result.vuln_chart = data.vuln_chart || undefined;
	...
	if (data.company) {
		...
		result.company.shortName =
			data.company.shortName || result.company.name;
	...
	for (finding of data.findings) {
		...
		if (tmpFinding.remediationComplexity === 1)
			tmpFinding.remediation.cellColorComplexity =
				cellLowColorRemediationComplexity;
		else if (tmpFinding.remediationComplexity === 2)
			tmpFinding.remediation.cellColorComplexity =
				cellMediumColorRemediationComplexity;
		else if (tmpFinding.remediationComplexity === 3)
			tmpFinding.remediation.cellColorComplexity =
				cellHighColorRemediationComplexity;
		else tmpFinding.remediation.cellColorComplexity = cellNoneColor;

		// Remediation Priority color
		if (tmpFinding.priority === 1)
			tmpFinding.remediation.cellColorPriority =
				cellLowColorRemediationPriority;
		else if (tmpFinding.priority === 2)
			tmpFinding.remediation.cellColorPriority =
				cellMediumColorRemediationPriority;
		else if (tmpFinding.priority === 3)
			tmpFinding.remediation.cellColorPriority =
				cellHighColorRemediationPriority;
		else if (tmpFinding.priority === 4)
			tmpFinding.remediation.cellColorPriority =
				cellUrgentColorRemediationPriority;
		...
		if (tmpCVSS.environmentalModifiedExploitability)
			tmpFinding.cvss.environmentalModifiedExploitability =
				CVSS31.roundUp1(tmpCVSS.environmentalModifiedExploitability);
